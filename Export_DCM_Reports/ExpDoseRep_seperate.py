...
def count_event_types(dicom_data):
    fluoro_count = 0
    dsa_count = 0

    def get_code_meaning(code_sequence):
        if code_sequence and (0x0008, 0x0104) in code_sequence[0]:
            return code_sequence[0][(0x0008, 0x0104)].value
        return None

    def extract_event_type(event_item):
        if (0x0040, 0xA730) not in event_item:
            return None
        for sub_item in event_item[(0x0040, 0xA730)].value:
            if (0x0040, 0xA043) in sub_item:
                concept_name_code_sequence = sub_item[(0x0040, 0xA043)].value
                code_meaning = get_code_meaning(concept_name_code_sequence)
                if code_meaning == 'Irradiation Event Type':
                    if (0x0040, 0xA168) in sub_item:
                        value_meaning = get_code_meaning(sub_item[(0x0040, 0xA168)].value)
                        if value_meaning:
                            return value_meaning
                    if (0x0040, 0xA160) in sub_item:
                        return sub_item[(0x0040, 0xA160)].value
            if (0x0040, 0xA730) in sub_item:
                nested = extract_event_type(sub_item)
                if nested:
                    return nested
        return None

    def scan_sequence(sequence):
        nonlocal fluoro_count, dsa_count
        for item in sequence:
            if (0x0040, 0xA043) in item:
                concept_name_code_sequence = item[(0x0040, 0xA043)].value
                code_meaning = get_code_meaning(concept_name_code_sequence)
                if code_meaning == 'Irradiation Event X-Ray Data':
    ...
    if (0x0040, 0xA730) in dicom_data:
        scan_sequence(dicom_data[(0x0040, 0xA730)].value)

    return fluoro_count, dsa_count

def read_dicom_files(folder_path, progress=None):
    ...
    for file in dicom_files:
        ...
        if ((sop_class_uid and (
            ...
            fluoro_events, dsa_events = count_event_types(dicom_data)
            ...
            data_total.append({'Patient Name': pname,
                    ...
                    'Fluoro Events': fluoro_events,
                    'DSA Events': dsa_events})
        else:
            pass
...
def report_progress(_file_path):
    ...
    processed_count += 1
    print(f"Processed file: {processed_count}", flush=True)

for pdir in patient_dirs:
    dftotal, pname = read_dicom_files(pdir, progress=report_progress)
    #--------------------
...
if total:
    with pd.ExcelWriter(output_directory, engine='openpyxl') as writer:
        ...
        numeric_cols = [
            ...
            'Dose (RP) Total (Gy)',
            'Fluoro Dose Area Product Total (Gym²)',
