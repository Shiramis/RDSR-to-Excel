    ...
    def startpro(self,word_file, index):
        ...
        if o == 1:
         ...
         ###### make all list without words######
            ...
            #if self.manufacturer == "Siemens AXIOM-Artis":
            ...
            '''else:
                self.all_data = {"Dose Area Product (Gym\u00b2)": self.dap1, "Dose (RP) (Gy)": self.drp1,
                                  "Positioner Primary Angle (deg)": self.ppa1,
                                 "Positioner Secondary Angle (deg)": self.psa1,"X-Ray Filter Material": self.xfm,
                                 'X-Ray Filter Thickness Maximum (mmCu)': self.xrftm1, "Pulse Rate (pulse/s)": self.pr1,
                                 'Irradiation Duration (s)': self.ird1,
                                 'KVP': self.kvp1, 'X-Ray Tube Current (mA)': self.xrtc1, 'Exposure Time (ms)': self.ext1,
                                 'Pulse Width (ms)': self.pulw1, 'Exposure (uA.s)': self.exp1,'Collimated Field Area (m\u00b2)': self.cfa1,
                                 'Collimated Field Height (mm)': self.cfh1, 'Collimated Field Width (mm)': self.cfw1,
                                  'Distance Source to Detector (mm)': self.dstd1}'''
            ...
            '''else:
                self.data_total = {"Patient ID": ID, "Manufacturer": self.manufacturer, "Content Date": self.content,
                                   "Person Observer Name": self.obs, "Dose Area Product Total (Gym\u00b2)": self.total[0],
                                   "Dose (RP) Total (Gy)": self.total[1],
                                   "Fluoro Dose Area Product Total (Gym\u00b2)": self.total[2],
                                   "Fluoro Dose (RP) Total (Gy)": self.total[3],"Distance Source to Reference Point (mm)":self.distance[0], "Total Fluoro Time (s)": self.time[0],
                                   "Acquisition Dose Area Product Total (Gym\u00b2)": self.total[4],
                                   "Acquisition Dose (RP) Total (Gy)": self.total[5],
                                   "Reference Point Definition (cm)": self.rpd[0],
                                   "Total Acquisition Time (s)": self.time[1]}'''
...
def convert_and_append(source_list, target_list):
    ...
    for item in source_list:
        if item == "N/A":
            ...
            result_list.append("N/A")  # Handle non-string, non-"N/A" values
